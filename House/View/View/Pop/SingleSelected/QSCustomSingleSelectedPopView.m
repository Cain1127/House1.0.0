...
typedef enum
{
    ...
    sSinglePickConfirmTypeHave = 99,        //!<显示确认按钮
    sSinglePickConfirmTypeWithout,          //!<不显示确认按钮

}SINGLE_PICK_CONFIRM_TYPE;
...
@property (nonatomic,assign) SINGLE_PICK_CONFIRM_TYPE confirmType;  //!<类型
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    return [self initWithFrame:frame andConfirmType:sSinglePickConfirmTypeHave];

}

- (instancetype)initWithFrame:(CGRect)frame andConfirmType:(SINGLE_PICK_CONFIRM_TYPE)confirmType
{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        [self createCustomPopviewInfoUIWithType];
...
#pragma mark - UI搭建
- (void)createCustomPopviewInfoUIWithType
{
    ...
    ///添加确定和不限
    if (sSinglePickConfirmTypeHave == self.confirmType) {
        
        ///不限
        QSCustomSelectedView *unlimitedView = [[QSCustomSelectedView alloc] initWithFrame:CGRectMake(0.0f, ypoint, rootView.frame.size.width, VIEW_SIZE_NORMAL_BUTTON_HEIGHT) andSelectedInfo:@"不限" andSelectedType:cCustomSelectedViewTypeSingle andSelectedBoxTapCallBack:^(BOOL currentStatus) {
        ...
        ///重置开始坐标
        ypoint = unlimitedView.frame.size.height + 4.0f;
        bottomHeight = 88.0f;
        
    }
    
    ///其他选择项的底view
    UIScrollView *selectedItemRootView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.0f,ypoint, rootView.frame.size.width, rootView.frame.size.height - ypoint - bottomHeight)];
...
- (void)createSingleSelectedInfoUI:(NSArray *)dataSource andCurrentSelectedKey:(NSString *)selectedKey
{
    ...
    switch (self.confirmType) {
            ///有确认
        case sSinglePickConfirmTypeHave:
            
            [self createSingleSelectedHaveConfirmInfoUI:dataSource andCurrentSelectedKey:selectedKey];
            
            break;
            
            ///无确认
        case sSinglePickConfirmTypeWithout:
            
            [self createSingleSelectedWithoutConfirmInfoUI:dataSource andCurrentSelectedKey:selectedKey];
...
- (void)createSingleSelectedWithoutConfirmInfoUI:(NSArray *)dataSource andCurrentSelectedKey:(NSString *)selectedKey
{
    ...
    for (int i = 0; i < [dataSource count]; i++) {
        ...
        UIButton *selectedButton = [UIButton createBlockButtonWithFrame:CGRectMake(0.0f, i * VIEW_SIZE_NORMAL_BUTTON_HEIGHT, rootView.frame.size.width, VIEW_SIZE_NORMAL_BUTTON_HEIGHT) andButtonStyle:buttonStyle andCallBack:^(UIButton *button) {
            
            ///回调
            if (self.customPopviewTapCallBack) {
                
                ///将当前选择的信息回调
                self.customPopviewTapCallBack(cCustomPopviewActionTypeSingleSelected,tempModel,i);
        ...
        [rootView addSubview:selectedButton];
...
- (void)createSingleSelectedHaveConfirmInfoUI:(NSArray *)dataSource andCurrentSelectedKey:(NSString *)selectedKey
{
...
+ (instancetype)showSingleSelectedViewWithDataSource:(NSArray *)dataSource andCurrentSelectedKey:(NSString *)selectedKey andSelectedCallBack:(void(^)(CUSTOM_POPVIEW_ACTION_TYPE actionType,id params,int index))selectedCallBack
{
    ...
    QSCustomSingleSelectedPopView *singleSelectedView = [[QSCustomSingleSelectedPopView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, SIZE_DEVICE_WIDTH, SIZE_DEVICE_HEIGHT) andConfirmType:sSinglePickConfirmTypeHave];
...
+ (instancetype)showSingleSelectedViewNoConfirmWithDataSource:(NSArray *)dataSource andCurrentSelectedKey:(NSString *)selectedKey andSelectedCallBack:(void(^)(CUSTOM_POPVIEW_ACTION_TYPE actionType,id params,int index))selectedCallBack
{
    ...
    QSCustomSingleSelectedPopView *singleSelectedView = [[QSCustomSingleSelectedPopView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, SIZE_DEVICE_WIDTH, SIZE_DEVICE_HEIGHT) andConfirmType:sSinglePickConfirmTypeWithout];
