//
//  QSPSalerTransactionOrderListViewController.m
//  House
//
...
#import "QSPSalerTransactionOrderListPendingView.h"
...
@interface QSPSalerTransactionOrderListViewController ()

@property (nonatomic,assign) MYZONE_SALER_TRANSACTION_ORDER_LIST_TYPE selectedListType;//!<当前选中显示的列表状态

@end

@implementation QSPSalerTransactionOrderListViewController
...
- (instancetype)init
{
    ...
    if (self = [super init]) {
        ...
        ///获取当前用户类型
        self.selectedListType = mSalerTransactionOrderListTypePending;
    }
...
- (void)createMainShowUI
{
    ...
    ///创建待成交列表
    QSPSalerTransactionOrderListPendingView *pendingListView = [[QSPSalerTransactionOrderListPendingView alloc] initWithFrame:CGRectMake(0, 104.0f, SIZE_DEVICE_WIDTH, mainHeightFloat - 40.0f)];
    ...
    ///创建已成交列表
    QSPSalerTransactionOrderListCompletedView *completeListView = [[QSPSalerTransactionOrderListCompletedView alloc] initWithFrame:CGRectMake(SIZE_DEVICE_WIDTH, pendingListView.frame.origin.y, pendingListView.frame.size.width, pendingListView.frame.size.height)];
    ...
    ///创建已取消列表
    QSPSalerTransactionOrderListCancelView *cancelListView = [[QSPSalerTransactionOrderListCancelView alloc] initWithFrame:CGRectMake(SIZE_DEVICE_WIDTH, pendingListView.frame.origin.y, pendingListView.frame.size.width, pendingListView.frame.size.height)];
    ...
    PendingButton = [UIButton createBlockButtonWithFrame:CGRectMake(0.0f, 64.0f, SIZE_DEVICE_WIDTH / 3.0f, 40.0f) andButtonStyle:buttonStyle andCallBack:^(UIButton *button) {
        ...
        [self showAllList];
        ///转换选择状态
        [self setSelectedType:mSalerTransactionOrderListTypePending animateWithDuration:0.3f];
    ...
    completeButton = [UIButton createBlockButtonWithFrame:CGRectMake(SIZE_DEVICE_WIDTH / 3.0f, 64.0f, SIZE_DEVICE_WIDTH / 3.0f, 40.0f) andButtonStyle:buttonStyle andCallBack:^(UIButton *button) {
        ...
        [self showAllList];
        ///转换选择状态
        [self setSelectedType:mSalerTransactionOrderListTypeCompleted animateWithDuration:0.3f];
    ...
    cancelButton = [UIButton createBlockButtonWithFrame:CGRectMake(SIZE_DEVICE_WIDTH / 3.0f * 2, 64.0f, SIZE_DEVICE_WIDTH / 3.0f, 40.0f) andButtonStyle:buttonStyle andCallBack:^(UIButton *button) {
        ...
        [self showAllList];
        ///转换选择状态
        [self setSelectedType:mSalerTransactionOrderListTypeCancel animateWithDuration:0.3f];
...
- (void)setSelectedType:(MYZONE_SALER_TRANSACTION_ORDER_LIST_TYPE)type animateWithDuration:(NSTimeInterval)duration
{
    ...
    QSPSalerTransactionOrderListPendingView *pendingListView = objc_getAssociatedObject(self, &PendingListTableViewKey);
    QSPSalerTransactionOrderListCompletedView *completeListView = objc_getAssociatedObject(self, &CompleteListTableViewKey);
    QSPSalerTransactionOrderListCancelView *cancelListView = objc_getAssociatedObject(self, &CancelListTableViewKey);
    
    switch (self.selectedListType) {
        case mSalerTransactionOrderListTypePending:
        {
            ///转换选择状态
            if (completeButton) {
            ...
            break;
        case mSalerTransactionOrderListTypeCompleted:
        {
            ///转换选择状态
            if (completeButton) {
            ...
            break;
        case mSalerTransactionOrderListTypeCancel:
        {
            ///转换选择状态
            if (completeButton) {
                completeButton.selected = NO;
...
- (void)showListWithType:(MYZONE_SALER_TRANSACTION_ORDER_LIST_TYPE)type
{
